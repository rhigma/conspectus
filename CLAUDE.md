...
Conspectus is a self-hosted personal AI assistant for school administrators. It is process-centric (not email-centric), organizing work around *Vorgänge* (processes/cases) and integrating email, calendar, notes, and delegations. The UI and much of the codebase is in German.
...
- `src/db.js` — MariaDB connection pool + schema initialization (`vorgaenge`, `emails`, `delegationen`, `events`, `calendars`, `email_accounts`, `vorgang_eintraege`, `chat_messages`, `sync_log`). Schema migrations use `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`.
- `src/ai.js` — Claude API integration; routes requests to `claude-haiku-4-5-20251001` (fast: summaries, search, email analysis) or `claude-sonnet-4-6` (smart: vision/handwriting); dynamic system prompt includes open tasks, overdue delegations, next 7 days of events; AI can trigger JSON-encoded *actions* (create Vorgang, assign email, etc.); `emailEinordnen()` returns `{einordnung, vorgang_id, vorgang_titel, begruendung, prioritaet, schlagworte[], ki_prioritaet}`.
...
- `src/caldav.js` — CalDAV sync with TZID-aware timezone parsing for Europe/Berlin.
...
- `src/pushover.js` — Push notifications via Pushover API.

### Frontend (`frontend/index.html`)

A single ~4300-line vanilla JS SPA (no framework, no build). Tabs: Dashboard, Vorgänge, E-Mails, KI-Assistent, Einstellungen. Uses CSS custom properties (`--ink`, `--accent`, etc.) and flexbox/grid.
...
**Key patterns:**
- `req(method, path, body)` — fetch wrapper that reads JSON error bodies and includes them in the thrown Error message
